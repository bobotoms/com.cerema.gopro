	...
	AddItem_click: function(me) {
		...
		App.Elements.getSelect(node.data.id,me.up('window').type_item,function(r){
			...
			for (var i=0;i<r.length;i++) {
				console.log(r[i].id);
				if (r[i].parent>0) var xnode=App.get(me.up('window'),"treepanel#T1").getRootNode().store.getNodeById(r[i].parent);
				if (!xnode) App.get(me.up('window'),"treepanel#T1").getRootNode().appendChild(r[i]); else xnode.appendChild(r[i]);
